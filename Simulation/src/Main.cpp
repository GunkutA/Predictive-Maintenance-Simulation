...
template <typename T>
void createCSVFile(std::string fileName, T tempData)
{
	//create CSV file
...
int main()
{
	...
	std::unique_ptr<PowerSimulation> PS = std::make_unique<PowerSimulation>();
	char buffer[4096];
	...
	if (CREATE_POWER_SIM)
	{
		createCSVFile("PowerSimulation.csv", PS->createPowerSimulation());
	}
	...
	if (device.deviceRole == device.deviceClient)
	{
		...
		//std::vector<double>powerSimVector = PS->createPowerSimulation(); //Use this vector if you want to create and send PowerSimulation data
